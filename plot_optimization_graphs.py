...
def _plot_tradeoff(metrics_df: pd.DataFrame, title: str, out_path: Path) -> None:
    ...
    if "is_selected" in metrics_df.columns:
        selected_mask = metrics_df["is_selected"].astype(bool)
    else:
        selected_mask = np.zeros(len(metrics_df), dtype=bool)
...
def _plot_summary_bars(summary_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.bar(positions, summary_df["total_min"], color="#1f77b4")
    ...
    fig, ax = plt.subplots(figsize=(max(6, len(labels) * 0.9), 5))
    ax.bar(positions, summary_df["cost"], color="#2ca02c")
    ax.set_ylabel("Cost (currency units)")
